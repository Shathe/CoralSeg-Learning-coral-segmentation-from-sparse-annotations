...
flags.DEFINE_multi_integer('eval_crop_size', [2048,1024],
  ...
  with tf.device('/cpu:0'):
    ...
    with tf.Graph().as_default():
      with tf.device('/cpu:0'):
        ...
        print(predictions['semantic'])
        print(predictions['semantic'].get_shape())
        ...
        indices = tf.squeeze(tf.where(tf.less_equal(labels, dataset.num_classes - 1)), 1)  # ignore all labels >= num_classes
        labels = tf.cast(tf.gather(labels, indices), tf.int32)
        predictions = tf.gather(predictions, indices)


        metric_map[predictions_tag] = tf.metrics.mean_iou(
            predictions, labels, dataset.num_classes)
